...
/* USER CODE BEGIN EV */
extern GCC_Context_t GCC_Context;
/* USER CODE END EV */
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          {
            /* read database hash characteristic */
            /* if equals to the stored one, no need to restart serv/charac discovery */
      ...
      switch (ad_type)
      {
        ...
        case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:          
          if ((ad_length >= 7) && (p_adv_data[i + 4] == 0x02))
          {
            ...
            if(p_adv_data[i + 6] == FW_ID_GC_SERVER)
            {
              ...
              LOG_INFO_APP("-- GC server detected, db addr 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
...
{
  GCC_Button1TriggerReceived();
  return;
}

void APPE_Button2Action(void)
{
  GCC_Button2TriggerReceived();   
  return;
}

void APPE_Button3Action(void)
{
  GCC_Button3TriggerReceived();  
  return;
}
#endif
